...
const otp = async(phone)=>{
	...
	fetch(`http://182.18.170.179/api.php?username=citygareage&password=228110&to=${phone}&from=GARAGC&message="OTP For Your mobile verification is ${otp}. Team Mr. Bike Doctor CITY GARAGE PRIVATE LIMITED"&PEID=1701164086573753451&templateid=1707164871113048733`)
	//fetch(`https://sendsms.designhost.in/index.php/smsapi/httpapi/?uname=skdigi&password=654321&sender=ALERTS&tempid=&receiver=${phone}&route=TA&msgtype=1&sms=OTP For Your mobile verification is ${otp}. Team Mr. Bike Doctor CITY GARAGE PRIVATE LIMITED`)
    .then(res => res.text())
    .then(text => {
		console.log(text);
    });
